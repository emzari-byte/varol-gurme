...
class ControllerCommonMenu extends Controller {
	public function index() {
		...
		$data['yoreselname'] = $popular_section['name'];
	...
	private function getRandomNewestProductIds($pool_limit = 30, $display_limit = 5) {
		...
		$exclude_sunday_products = (date('D') == 'Sun');
		...
		$query = $this->db->query("SELECT DISTINCT p.product_id
			...
			" . ($exclude_sunday_products ? "AND p.sku <> '99'" : "") . "
		...
		return array_slice($product_ids, 0, $display_limit);
	}
